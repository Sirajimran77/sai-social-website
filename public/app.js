...
(function () {
  ...
  const privacySections = [
    ...
    { n: '04', h: 'Cookies & tracking', body: 'This site uses Google Analytics to understand which pages bring in enquiries, and Google Ads conversion measurement to see which adverts lead to a booking. These set cookies on your device, so we ask before we turn them on: analytics and advertising cookies stay switched off until you accept, and if you decline, none are set. Everything else the site uses is strictly necessary to serve the pages and keep them secure. Google may process this data outside the UK — see "International transfers" below. You can change your mind at any time using the cookie settings button below, or by controlling cookies through your browser.' },
  ...
  /* ------------------------------------------------------- COOKIE CONSENT */
  // The tag state lives in /gtag-init.js, which is generated by the server and
  // already defaults Consent Mode to "denied". All this does is ask, then pass
  // the answer along. If no tracking tag is configured, .required is false and
  // the banner never appears — we don't ask permission for cookies we don't set.
  (function consentBanner() {
    const bar = $('#cookieBar');
    const consent = window.saiConsent;
    if (!bar || !consent || !consent.required) return;

    const close = () => bar.removeAttribute('data-open');
    const open = () => bar.setAttribute('data-open', '');

    // Only meaningful when there are tags to switch off, so it stays hidden
    // otherwise rather than offering a control that does nothing.
    const control = $('#privacyCookieControl');
    if (control) control.hidden = false;

    $('#cookieAccept').addEventListener('click', () => { consent.grant(); close(); });
    $('#cookieDecline').addEventListener('click', () => { consent.deny(); close(); });

    // Withdrawing consent has to stay possible after a decision, so this is
    // bound regardless of current state — it is the people who already chose
    // who need it. Delegated because the privacy view's markup is rendered
    // from privacySections after this runs.
    document.addEventListener('click', (e) => {
    ...
    // Only prompt when there's no decision on file.
    if (consent.state !== 'granted' && consent.state !== 'denied') open();
  })();
